...
(function() {
  function init(e) {
    ...
    var versions = ['2.3.0', '2.3.1', '2.3.2', '2.3.3', '2.3.4', '2.3.5', '2.3.6', '2.3.7', '2.3.8', '2.3.9', '2.3.10', '2.3.11', '2.4.0', '2.5.0'];
    ...
    // http://geoservices.informatievlaanderen.be/raadpleegdiensten/administratieve_eenheden/wms?request=getcapabilities&service=wms&version=1.3.0
    checkVersion();
    ...
    queryWindow.style.overflow = 'auto';
          ...
          onload: function(response) {
            ...
            if (response.status == 200) {
              ...
              if (body && content.length != 0) {
                ...
                queryWindow.style.display = 'block';
                var clickHandler = exportFunction(function(e) {
                  queryWindow.style.display = 'none';
                  document.getElementById('WazeMap').parentNode.removeEventListener('click', clickHandler);
                  e.stopPropagation();
                }, unsafeWindow);
                document.getElementById('WazeMap').parentNode.addEventListener('click', clickHandler);
              } else {
    ...
    function removeUnsafeAttributes(node) {
      if (node.nodeType == Node.ELEMENT_NODE) {
        for (var i = 0; i < node.attributes.length; i++) {
          ...
          if (attrName.startsWith('on') || attrName == 'style' || attrName == 'class' || attrName == 'xmlns' || (attrName == 'href' && node.attributes[i].value.trim().toLowerCase().startsWith('javascript:'))) {
    ...
    function reorderTableRows(body, map) {
      ...
      var rows = body.querySelectorAll('table tr');
      for (var i = 0; i < rows.length; i++) {
        if (propertyOrder[map.id].indexOf(rows[i].firstChild.textContent) > -1) {
          rows[i].parentNode.insertBefore(rows[i], rows[i].parentNode.firstChild);
        }
      }
    }
